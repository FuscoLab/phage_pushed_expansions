function noIads_Solver(name, Ktemp, Ltemp, alphatemp)
%This is the code that is actually run to solve the pdes for a given K, L
%and alpha value (equivalent to providing f and Kmax since Bmax fixed).
%It should run the solver, and then save the bacteria/virus profiles
...
%veolicty can be determined "manually" using a different code.
    ...
    %Solve pdes
    m=0;
    sol=pdepe(m,@noIads3,@variable3ic,@variable3bc,x,t);
    ...
    save([name '.mat'],'avv', 'avw_B', 'avw_I', 'b', 'i', 'v', 'K', 'L', 'v_asymptote');
