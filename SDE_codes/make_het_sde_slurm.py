...
alphas = np.array([.005])#alpha values 
Bs = np.array([150])#B0 values (need corresponding script for each value in dir. )
tausl =4
taus=np.zeros((len(alphas),tausl))
aBt_0 =750*.005#50*.03
aBt_f = 1200*.005#200*.03
taus = [[750,900,1050,1200]]
samps = 1000
files= []
...
for a,alpha in enumerate(alphas):
	for tau in taus[a]:
		for B in Bs:
			...
			f.write('g++ -o outanc_UDMp_'+param_str+'_$SLURM_ARRAY_TASK_ID phage_inf_coarse_het_Nb'+str(B)+'_UDM_plus.cpp -std=c++11\n')
			f.write('./outanc_UDMp_'+param_str+'_$SLURM_ARRAY_TASK_ID -t '+str(tau)+' -a '+str(alpha)+' -i $SLURM_ARRAY_TASK_ID\n')
			f.close()
